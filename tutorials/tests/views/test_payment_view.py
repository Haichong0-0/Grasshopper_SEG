...
class ConfirmRejectClassViewTests(TestCase):
    ...
    def test_confirm_class_view(self):
        ...
        self.assertEqual(availability.starttime, datetime.strptime(str(self.lesson.start_time), '%H:%M:%S').time())
    ...
    def test_unauthenticated_access(self):
        ...
        # Assert access is redirected for unauthenticated users
        self.assertEqual(confirm_response.status_code, 302)  # Redirect
        self.assertEqual(reject_response.status_code, 302)  # Redirect
    ...
    def test_non_admin_access(self):
        ...
        # Assert access is redirected for non-admin users
        self.assertEqual(confirm_response.status_code, 302)  # Redirect
        self.assertEqual(reject_response.status_code, 302)  # Redirect

    def test_reject_class_invalid_data(self):
        """Test reject class with invalid data."""
        url = reverse("reject_class", kwargs={"lesson_id": self.lesson.lesson_id})
        response = self.client.post(url, {})  # Empty data

        self.lesson.refresh_from_db()
        self.assertEqual(response.status_code, 302, "Invalid data should still redirect.")
        self.assertEqual(self.lesson.status, "Rejected", "Lesson should be marked as Rejected.")